...
bool NetworkManager::Init()
{
	...
	int i = 0; //Not necessary in normal conections but in labs loop dont end
	do
	{
		netManager->country = netManager->GetIPCountry();
		netManager->myIP = netManager->findMyIP();
		i++;
	} while (netManager->country == "--" && i < 100);

	//std::cout << "Your country: " << netManager->country << std::endl;
	//std::cout << "Your public IP: " << netManager->myIP << std::endl;
...
std::string NetworkManager::GetIPCountry() {
	...
	{
		std::cout << "Error: IP not found" << std::endl;
		sol = "--";
	}
	return sol;
}
...
std::string NetworkManager::findMyIP()
{
	std::string myIP;

	CURL* curl = curl_easy_init();
	if (curl) {
		curl_easy_setopt(curl, CURLOPT_URL, "http://ipinfo.io/ip?token=9df91f2f5e395c");
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, GetInfo);
